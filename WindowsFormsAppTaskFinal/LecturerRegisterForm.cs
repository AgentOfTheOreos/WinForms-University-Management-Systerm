...
namespace WindowsFormsAppTaskFinal
{
    public partial class LecturerRegisterForm : Form
    {
        ...
        private void finishRegButton_Click_1(object sender, EventArgs e)
        {
            ...
            var employeeNumber = GenerateEmployeeNumber();
            var selectedCourses = GetListBoxItemsAsList(coursesListBox);
            var isValid = true;
            ...
            var personExists = _trackDirector.Persons.Any(person => person.Email.Equals(email) || person.Id == id);
                ...
                foreach (var courseName in selectedCourses)
                {
                    ...
                    {
                        MessageBox.Show(
                            @"The academic track that the 
                                    entered course(s) belong to either does not exist anymore,
                                    or you have entered the name(s) of (a) course(s) that do not exist in the system,
        ...
        private int GenerateEmployeeNumber()
        {
            ...
            do
            {
                ...
                uniqueNumberNotFound = (from person in _trackDirector.Persons
                    where person.SubclassIdentifier.Equals("Lecturer")
                    select (Lecturer)person).Any(lect => lect.EmployeeNumber == employeeNumber);
        ...
        private static List<string> GetListBoxItemsAsList(ListBox listBox)
        {
